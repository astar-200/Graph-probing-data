...
`-- openwebtext # openwebtext通用数据集对应的神经拓扑图
    ...
    |-- cross_layer
    |   `-- full # 包含了模型全部层级全部神经元两两相关信息的拓扑图（取前100个句子的平均）
    |   `-- cross_layer_x_y_avg_corr.npy # 第x和第y层的神经拓扑图 （取前100个句子的平均）
    `-- layer12 # 只计算模型第12层的数据
        |-- average_100 # 对前100条数据得到的平均图做平均
        |-- partitioned_100 # 将前100条数据均分为3份，每份内做平均，得到局部图
...
目前本仓库提供的图为稠密图，保留了全部神经元之间的相关系数。实际使用中可以适当做稀疏化
...
2026.3.10：添加了openwebtext数据全部层级的神经拓扑图、任意两层间的神经拓扑图以及包含模型全部神经元的神经拓扑大图（使用前100条句子计算平均图）
